...
void figure_8_test(list_raft_group* group, int num_tries = 1000)//原来是1000
{
    int num_nodes = 5;
    ...
    for (int iters = 0; iters < num_tries; iters++) {
        ...
        // ).count();
                group->restart(s);
        //         unsigned long b=std::chrono::duration_cast<std::chrono::milliseconds>(
        //     std::chrono::system_clock::now().time_since_epoch()
        // ).count();
        // printf("restart time %d,%d,%d \n",b-a,a,b);
                nup ++;
            }
        }
    }
...
TEST_CASE(part3, unreliable_agree, "Agreement under unreliable network")
{
    ...
    for (int iters = 1; iters < 50; iters++) {//原来是 50
        // printf("iters %d begin!\n",iters);
        for (int j = 0; j < 4; j++) {
