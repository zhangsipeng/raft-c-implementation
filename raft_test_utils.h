    ...
    for (size_t i = 0; i < nodes.size(); i++) {
        ...
        if (has_log) {
            ...
            if (cnt == 1) {
                ...
                ASSERT(old_value == log_value, "inconsistent log value: (" << log_value << ", " << old_value << ") at idx " << log_idx<<"new node"<<i<<"old node"<<oldidx);
            }
        }
    }
    return cnt;
        ...
        if (log_idx != -1) {
            ...
            while (std::chrono::system_clock::now() < check_start + std::chrono::seconds(2)) {
                ...
                if (committed_server >= expected_servers) {
                    ...
                    // else{
                    //     std::cout<<"commited"<<commited_value<<"actual"<<std::endl<<value;
                    // }
                }
                mssleep(20);
            }
